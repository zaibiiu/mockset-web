...
namespace Botble\Rental\Supports;
...
class InvoiceHelper
{
    public static function store(array $data = []): bool
    {
        ...
        $reference = Booking::query()->whereIn('id', $orderIds)->first();
        $user = $reference->bookingable;
        $discount = !empty((int)$reference->discount) ? $reference->discount : $reference->applicable_discount;
        $invoice = new Invoice([
            'account_id' => $reference->bookingable_id,
            'sub_total' => $amount + $discount,
            'fees' => $reference->fees,
            'tax_amount' => 0,
            'shipping_amount' => 0,
            'discount_amount' => $discount ?? 0,
            'amount' => $amount + $reference->fees,
        ...
        if ($reference) {
            ...
            $invoice->invoiceable()->associate($user);

            $invoice->save();

            $invoice->items()->create([
                'name' => $reference->vehicle->name,
                'description' => null,
                'qty' => 1,
                'sub_total' => $amount + $discount,
                'fees' => $reference->fees,
                'tax_amount' => 0,
                'discount_amount' => $reference->discount,
                'amount' => $amount,
            ]);
        }
