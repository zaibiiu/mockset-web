@if (is_payment_gateway_active('paypal'))
    <li class="list-group-item">
        ...
        <label for="payment_paypal" class="text-start">
            <img class="filter-black" src="{{ url('vendor/core/plugins/paypal/images/paypal.svg') }}"
                 alt="{{ setting('payment_paypal_name', trans('plugins/payment::payment.payment_via_paypal')) }}">
        </label>
        <div class="payment_paypal_wrap payment_collapse_wrap collapse @if ($selecting == PAYPAL_PAYMENT_METHOD_NAME) show @endif"
            ...
            @if (function_exists('get_application_currency') && !in_array(get_application_currency()->title, $supportedCurrencies) && !get_application_currency()->replicate()->where('title', 'USD')->exists())
                <div class="alert alert-warning" style="margin-top: 15px;">
                    ...
                    <div style="margin-top: 10px;">
                        {{ __('Learn more') }}: <a href="https://developer.paypal.com/docs/api/reference/currency-codes"
                                                   target="_blank" rel="nofollow">https://developer.paypal.com/docs/api/reference/currency-codes</a>
                    </div>
