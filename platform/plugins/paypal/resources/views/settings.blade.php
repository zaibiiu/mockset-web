...
@php
    $payPalSettingsRepo = app(\Botble\Payment\Repositories\Interfaces\PaymentGatewaySettingInterface::class);
    $payPalSettings = $payPalSettingsRepo->getFirstBy(['payment_gateway' => 'paypal']);
    $data = [];
    $data['mode'] = 0;
    if(!empty($payPalSettings)){
        $data = $payPalSettings->toArray();
    }
@endphp
    ...
    <tr class="payment-content-item hidden">
        <td class="border-left" colspan="3">
                        ...
                        <li class="payment-note">
                            ...
                            <ul class="m-md-l" style="list-style-type:decimal">
                                <li style="list-style-type:decimal">
                                    <a href="https://www.paypal.com/vn/merchantsignup/applicationChecklist?signupType=CREATE_NEW_ACCOUNT&amp;productIntentId=email_payments"
                                       target="_blank">
                ...
                <div class="col-sm-6">
                    <div class="well bg-white">
                        <x-core-setting::text-input
                                ...
                                :value="isset($data['name'])?$data['name']:trans('plugins/payment::payment.pay_online_via', ['name' => 'PayPal'])"
                                data-counter="400"
                        />

                        <div class="form-group mb-3">
                            <label class="text-title-field"
                                   for="payment_paypal_description">{{ trans('core/base::forms.description') }}</label>
                            <textarea class="next-input" name="payment_paypal_description"
                                      id="payment_paypal_description">{{ isset($data['description'])?$data['description']:__('You will be redirected to PayPal to complete the payment.')}}</textarea>
                        </div>

                        <p class="payment-note">
                            {{ trans('plugins/payment::payment.please_provide_information') }}
                            <a target="_blank"
                               href="//www.paypal.com">PayPal</a>:
                        </p>

                        <x-core-setting::text-input
                                name="payment_paypal_client_id"
                                :label="trans('plugins/payment::payment.client_id')"
                                :value="isset($data['client_key'])?$data['client_key']:''"
                        />
                        ...
                        <x-core-setting::text-input
                                ...
                                :value="isset($data['client_secret'])?$data['client_secret']:''"
                        />
            ...
            <div class="col-12 bg-white text-end">
                <button class="btn btn-warning disable-payment-item @if ($payPalStatus == 0) hidden @endif"
                        type="button">{{ trans('plugins/payment::payment.deactivate') }}</button>
                <button class="btn btn-info save-payment-item btn-text-trigger-save @if ($payPalStatus == 1) hidden @endif"
                        type="button">{{ trans('plugins/payment::payment.activate') }}</button>
                <button class="btn btn-info save-payment-item btn-text-trigger-update @if ($payPalStatus == 0) hidden @endif"
