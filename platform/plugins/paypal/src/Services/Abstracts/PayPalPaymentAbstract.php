...
abstract class PayPalPaymentAbstract
{
    ...
    public function environment(): SandboxEnvironment|ProductionEnvironment
    {
        ...
        $paypalSettings = app(PaymentGatewaySettingInterface::class)
            ->getFirstBy(['payment_gateway' => 'paypal', 'status' => ActiveInactiveStatusEnum::ACTIVE]);

        if (!empty($paypalSettings)) {
            $clientId = $paypalSettings->client_key;
